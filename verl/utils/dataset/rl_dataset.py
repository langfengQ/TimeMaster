...
# def process_image(image: dict, max_pixels: int = 2048 * 2048, min_pixels: int = 256 * 256):
#     import math
...
#     # try:
#     #     image = Image.open(image).convert('RGB')  # 转成 RGB，避免灰度图或 alpha 报错
...
#     return image
def process_image(image_info: dict, max_pixels: int = 2048 * 2048, min_pixels: int = 256 * 256):
    from io import BytesIO
    from PIL import Image
    import math

    # 直接从 bytes 构建图像，不做多余转换
    with BytesIO(image_info['bytes']) as buf:
