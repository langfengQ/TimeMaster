...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def _safe_load_optimizer_state(self, optimizer, checkpoint_state_dict, verbose=True):
        current_param_ids = {id(p): p for group in optimizer.param_groups for p in group['params']}
        
        filtered_state = {}
        for param_id, state in checkpoint_state_dict['state'].items():
            if param_id in current_param_ids:
                filtered_state[param_id] = state
                print(f"✅ Loading optimizer param id: {param_id}")
            elif verbose:
                print(f"⚠️ Skipping unmatched optimizer param id: {param_id}")

        # 保留当前 param_groups 结构
        new_state_dict = {
        ...
        if verbose:
            print("✅ Optimizer state loaded partially (only matching parameters).")
    ...
    def load_checkpoint(self, path=None, del_local_after_load=False, *args, **kwargs):
        ...
        with FSDP.state_dict_type(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            ...
            if self.optimizer is not None:
               # self._safe_load_optimizer_state(self.optimizer, optimizer_state_dict)
